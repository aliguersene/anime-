...
header{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:14px;border-radius:12px;background:linear-gradient(180deg,rgba(255,255,255,0.02),transparent);backdrop-filter:blur(6px);position:sticky;top:12px;z-index:90;flex-wrap:wrap}
...
.search input{background:transparent;border:0;outline:none;color:var(--text);width:220px;max-width:100%}
...
.card{display:flex;gap:12px;align-items:center;padding:12px;border-radius:12px;background:linear-gradient(180deg, rgba(255,255,255,0.015), rgba(255,255,255,0.01));border:1px solid rgba(255,255,255,0.02);transition:transform .28s ease,box-shadow .28s;opacity:0;transform:translateY(18px);flex-wrap:wrap}
...
.card .stars{display:flex;gap:6px;margin-top:8px;flex-wrap:wrap}
...
.episode{display:flex;justify-content:space-between;align-items:center;padding:10px;border-radius:10px;background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.02);margin-bottom:8px;flex-wrap:wrap}
...
.modal-row{display:flex;gap:18px;flex-wrap:wrap}
.modal-row img{width:360px;height:360px;object-fit:cover;border-radius:10px;max-width:100%;height:auto}
...
@media (max-width:480px){
  ...
  #botPopup{width:90%;right:5px;bottom:80px;max-height:60vh;}
  #botIcon{width:56px;height:56px;font-size:26px;bottom:10px;right:10px;}
}
.muted{color:var(--muted)}
</style>
</head>
<body data-theme="dark">
<!-- المحتوى نفسه كما أرسلته -->
<!-- HTML + JS بدون تغيير -->
</body>
</html>
